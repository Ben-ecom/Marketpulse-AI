...
/**
 * Component voor het visualiseren van pijnpunten data van verschillende platforms
 ...
 */
const PainPointsChart = ({ platform = 'all', data = {}, height = 400 }) => {
  ...
  const getPlatformData = () => {
    ...
    if (platform === 'all') {
      ...
      return data.platforms.reduce((acc, platformData) => {
        if (platformData.painPoints && platformData.painPoints.byCategory) {
          Object.entries(platformData.painPoints.byCategory).forEach(([category, items]) => {
      ...
      if (!platformData || !platformData.painPoints || !platformData.painPoints.byCategory) return [];
      
      return Object.entries(platformData.painPoints.byCategory).map(([category, items]) => ({
        category,
        count: items.length,
        platform
      }));
    ...
    return (
      <Box sx={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
        <Typography variant="body1" color="text.secondary" gutterBottom>
          Geen pijnpunten data beschikbaar voor {platform === 'all' ? 'alle platforms' : platform}
        </Typography>
  ...
  const getPlatformColor = (platformName) => {
    ...
    return colors[platformName] || theme.palette.error.main;
  };
    ...
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <Typography variant="h6" component="div">
          Pijnpunten per Categorie {platform !== 'all' ? `(${platform})` : ''}
        </Typography>
        <ToggleButtonGroup
          value={chartType}
          exclusive
      ...
      {chartType === 'bar' ? (
        <ResponsiveContainer width="100%" height={height}>
          ...
          >
            ...
            />
            <Tooltip
              formatter={(value, name, props) => [`Aantal: ${value}`, 'Aantal pijnpunten']}
              labelFormatter={(label) => `Categorie: ${label}`}
            />
            <Legend />
            <Bar dataKey="count" name="Aantal pijnpunten" radius={[0, 4, 4, 0]}>
          ...
          <PieChart>
            ...
            >
              {pieData.map((entry, index) => (
                <Cell 
                  key={`cell-${index}`} 
                  fill={platform === 'all' ? getPlatformColor(entry.platform) : theme.palette.error.main} 
                />
              ))}
            </Pie>
            <Tooltip />
            ...
            <CardContent>
              {platform === 'all' ? (
                data.platforms.map((platformData, index) => {
                  const painPointsCount = platformData.painPoints?.all?.length || 0;
                  return (
                    <Box key={index} display="flex" justifyContent="space-between" mb={1}>
                      <Typography variant="body2" sx={{ mr: 2 }}>
                ...
                <Typography variant="body2">
                  Totaal aantal pijnpunten voor {platform}: {chartData.reduce((sum, item) => sum + item.count, 0)}
                </Typography>
              )}
            </CardContent>
          </Card>
